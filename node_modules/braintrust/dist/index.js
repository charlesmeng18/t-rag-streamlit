...
var import_typespecs, import_eventsource_parser, import_zod, braintrustStreamChunkSchema, BraintrustStream;
var init_stream = __esm({
  "src/functions/stream.ts"() {
    "use strict";
    import_typespecs = require("@braintrust/core/typespecs");
    import_eventsource_parser = require("eventsource-parser");
    import_zod = require("zod");
    braintrustStreamChunkSchema = import_zod.z.union([
      import_zod.z.object({
      ...
      async evictOldest(entries) {
        const stats = await Promise.all(
          entries.map(async (entry) => {
            const stat2 = await isomorph_default.stat(this.getEntryPath(entry));
            return {
              name: entry,
        ...
        await Promise.all(
          toRemove.map((stat2) => isomorph_default.unlink(this.getEntryPath(stat2.name)))
        );
      }
    };
  }
...
function lintTemplate(template, context) {
  ...
  for (const variable of variables) {
    ...
    const fieldExists = (0, import_core.getObjValueByPath)(context, arrPathsReplaced.split(".")) !== void 0;
    if (!fieldExists) {
...
var import_core, import_mustache;
var init_mustache_utils = __esm({
  "src/mustache-utils.ts"() {
    "use strict";
    import_core = require("@braintrust/core");
    import_mustache = __toESM(require("mustache"));
  }
});
...
function updateSpan({
  ...
  const components = import_core2.SpanComponentsV3.fromStr(exported);
...
async function loadPrompt({
  ...
  try {
    ...
    const prompt2 = await state.promptCache.get({
      slug,
      projectId,
      projectName,
  ...
  const metadata = import_typespecs2.promptSchema.parse(response["objects"][0]);
...
var import_uuid, import_core2, import_typespecs2, import_functions, import_mustache2, import_zod2, NoopSpan, NOOP_SPAN, NOOP_SPAN_PERMALINK, loginSchema, stateNonce, BraintrustState, _globalState, _internalGetGlobalState, FailedHTTPResponse, HTTPConnection, BaseAttachment, Attachment, ExternalAttachment, attachmentMetadataSchema, ReadonlyAttachment, Logger, TestBackgroundLogger, HTTPBackgroundLogger, traceable, INTERNAL_BTQL_LIMIT, MAX_BTQL_ITERATIONS, ObjectFetcher, Experiment, ReadonlyExperiment, executionCounter, SpanImpl, Dataset, Prompt, TEST_API_KEY, _exportsForTestingOnly;
var init_logger = __esm({
  "src/logger.ts"() {
    "use strict";
    import_uuid = require("uuid");
    import_core2 = require("@braintrust/core");
    import_typespecs2 = require("@braintrust/core/typespecs");
    import_functions = require("@vercel/functions");
    import_mustache2 = __toESM(require("mustache"));
    ...
    loginSchema = import_zod2.z.strictObject({
    ...
    _internalGetGlobalState = () => _globalState;
    ...
    HTTPConnection = class _HTTPConnection {
      ...
      async get(path3, params = void 0, config) {
        ...
        const url = new URL((0, import_core2._urljoin)(this.base_url, path3));
      ...
      async post(path3, params, config) {
        ...
        const this_headers = this.headers;
        return await checkResponse(
          await this_fetch((0, import_core2._urljoin)(this_base_url, path3), {
            method: "POST",
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
...
with a Blob/ArrayBuffer, or run the program on Node.js.`
    ...
    attachmentMetadataSchema = import_zod2.z.object({
    ...
    TestBackgroundLogger = class {
      ...
      async drain() {
        ...
        const batch = (0, import_core2.mergeRowBatch)(events);
        return batch.flat();
      }
    };
      ...
      async unwrapLazyValues(wrappedItems) {
        for (let i = 0; i < this.numTries; ++i) {
          try {
            ...
            return [(0, import_core2.mergeRowBatch)(items), attachments];
          } catch (e) {
...
`,
    ...
    Experiment = class extends ObjectFetcher {
      lazyMetadata;
      dataset;
      lastStartTime;
      lazyId;
    ...
    Dataset = class extends ObjectFetcher {
      ...
      runBuild(buildArgs, options) {
        ...
        if (flavor === "chat") {
          ...
          return {
            ...
            messages,
            ...prompt.tools?.trim() ? {
              tools: import_typespecs2.toolsSchema.parse(JSON.parse(render(prompt.tools)))
            } : void 0
          };
          ...
          return {
            ...renderPromptParams(params, variables, { strict: options.strict }),
            ...spanInfo,
            prompt: import_mustache2.default.render(prompt.content, variables, void 0, {
              escape
            })
          };
        } else {
  ...
  return new Proxy(create, {
    apply(target, thisArg, argArray) {
      ...
      const spanArgs = {
        ...
        spanAttributes: {
          type: import_core6.SpanTypeAttribute.LLM,
          provider: "anthropic"
        },
        event: {
          input,
...
function apiPromiseProxy(apiPromise, span, onThen) {
...
function streamNextProxy(stream, sspan) {
  ...
  return async function(...args) {
    ...
    if (result.done) {
      totals.tokens = (totals["prompt_tokens"] || 0) + (totals["completion_tokens"] || 0);
      const output = deltas.join("");
      span.log({ output, metrics: totals, metadata });
      span.end();
      return result;
    }
...
var init_anthropic = __esm({
  "src/wrappers/anthropic.ts"() {
    "use strict";
    init_logger();
    import_core6 = require("@braintrust/core");
    init_util();
  }
});
...
function configureNode() {
  ...
  isomorph_default.gzip = (0, import_util2.promisify)(zlib.gzip);
  isomorph_default.gunzip = (0, import_util2.promisify)(zlib.gunzip);
...
async function runEvaluator(experiment, evaluator, progressReporter, filters, stream) {
  ...
  const q = queue(
    ...
    async (datum) => {
      ...
      const baseEvent = {
        name: "eval",
        spanAttributes: {
          type: import_core3.SpanTypeAttribute.EVAL
        },
        event: {
          input: datum.input,
      ...
      const callback = async (rootSpan) => {
        ...
        try {
          ...
          await rootSpan.traced(
            ...
            {
              name: "task",
              spanAttributes: { type: import_core3.SpanTypeAttribute.TASK },
                ...
                const results2 = await rootSpan.traced(
                  async (span) => {
                    ...
                    const resultMetadata = results3.length === 1 ? results3[0].metadata : results3.reduce(
                      (prev, s) => (0, import_core3.mergeDicts)(prev, {
                        [s.name]: s.metadata
                      }),
                      {}
                    );
                    const resultOutput = results3.length === 1 ? getOtherFields(results3[0]) : results3.reduce(
                      (prev, s) => (0, import_core3.mergeDicts)(prev, { [s.name]: getOtherFields(s) }),
                      {}
                    );
                    const scores2 = results3.reduce(
                      (prev, s) => (0, import_core3.mergeDicts)(prev, { [s.name]: s.score }),
                      {}
                    );
                    span.log({
                      output: resultOutput,
                  ...
                  },
                  {
                    name: scorerNames[score_idx],
                    spanAttributes: {
                      type: import_core3.SpanTypeAttribute.SCORE
                    },
...
function wrapBetaChatCompletionParse(completion) {
  return async (allParams) => {
    ...
    const span = startSpan(
      (0, import_core5.mergeDicts)(
        {
          name: "Chat Completion",
          spanAttributes: {
            type: import_core4.SpanTypeAttribute.LLM
          }
        },
...
function parseBaseParams(allParams, inputField) {
  ...
  return (0, import_core5.mergeDicts)(ret, { event: { input, metadata: paramsRest } });
}
...
// src/exports-node.ts
var wrapAnthropic2 = function wrapAnthropic3(anthropic) {
  return anthropic;
};
try {
  if (require.resolve("@anthropic-ai/sdk")) {
    wrapAnthropic2 = (init_anthropic(), __toCommonJS(anthropic_exports)).wrapAnthropic;
  }
} catch (e) {
}
